...
window.cssToken = function(name){ return getComputedStyle(document.documentElement).getPropertyValue(name).trim(); };
...
function renderChart(principal,interest){if(!window.Chart){return;}var canvas=document.getElementById('loanChart');var pPct=Math.round(principal/(principal+interest)*100);document.getElementById('chart-center-pct').textContent=pPct+'%';var ci=window._i18n_current;var lP=(ci&&ci['lbl-principal-pct'])||'Principal';var lI=(ci&&ci['lbl-interest-pct'])||'Interest';if(chartInstance){chartInstance.data.labels=[lP,lI];chartInstance.data.datasets[0].data=[principal,interest];chartInstance.update('none');return;}chartInstance=new Chart(canvas,{type:'doughnut',data:{labels:[lP,lI],datasets:[{data:[principal,interest],backgroundColor:[window.cssToken('--color-navy'),window.cssToken('--color-gold')],borderWidth:0,hoverOffset:6}]},options:{responsive:true,maintainAspectRatio:true,cutout:'68%',plugins:{legend:{display:false},tooltip:{callbacks:{label:function(ctx){return ctx.label+': '+fmt(ctx.raw)+' ('+Math.round(ctx.raw/(principal+interest)*100)+'%)';}}}},animation:{duration:400}}});}
...
function switchTab(tab){currentTab=tab;window._currentTab=tab;var cfg=CONFIGS[tab];document.querySelectorAll('.tab-btn[data-tab]').forEach(function(b){var active=b.dataset.tab===tab;b.classList.toggle('active',active);b.setAttribute('aria-selected',active);});var _ltp=document.getElementById('loan-type-content');if(_ltp)_ltp.setAttribute('aria-labelledby','loan-tab-'+tab);sAmount.min=cfg.amountMin;sAmount.max=cfg.amountMax;sAmount.step=cfg.amountStep;sAmount.value=cfg.amount;sRate.min=cfg.rateMin;sRate.max=cfg.rateMax;sRate.value=cfg.rate;sTerm.min=cfg.termMin;sTerm.max=cfg.termMax;sTerm.value=cfg.term;var _sym=window.getCurrencySym();document.getElementById('amount-min-lbl').textContent=_sym+fmtN(cfg.amountMin);document.getElementById('amount-max-lbl').textContent=_sym+fmtN(cfg.amountMax);document.getElementById('rate-min-lbl').textContent=cfg.rateMin+'%';document.getElementById('rate-max-lbl').textContent=cfg.rateMax+'%';var txYr=window._i18n_current&&window._i18n_current['unit-yr']||'yr';var txYrs=window._i18n_current&&window._i18n_current['unit-yrs']||'yrs';document.getElementById('term-min-lbl').textContent=cfg.termMin+' '+txYr;document.getElementById('term-max-lbl').textContent=cfg.termMax+' '+txYrs;var txL=function(k){return window._i18n_current&&window._i18n_current[k]||cfg.label;};document.getElementById('result-loan-label').textContent=txL('loan-label-'+tab);document.getElementById('helper-title').textContent=txL('helper-title-'+tab)||cfg.helperTitle;document.getElementById('helper-text').textContent=txL('helper-text-'+tab)||cfg.helperText;showAllRows=false;update();}
...
function updateCIChart(P,M,r,years){if(!window.Chart)return;var txCI=function(k){return window._i18n_current&&window._i18n_current[k]||k;};var lDep=txCI('ci-deposited');var lGrow=txCI('ci-earned-short');var lYr=txCI('unit-yr');var labels=[],depositsData=[],growthData=[];for(var y=1;y<=years;y++){var n=y*12;var fv=P*Math.pow(1+r,n);if(r>0){fv+=M*(Math.pow(1+r,n)-1)/r;}else{fv+=M*n;}var dep=P+M*n;labels.push(y===1?'1 '+lYr:(y%5===0?y+' '+lYr:''));depositsData.push(Math.round(dep));growthData.push(Math.round(Math.max(0,fv-dep)));}if(!ciChart){ciChart=new Chart(document.getElementById('ciChart'),{type:'bar',data:{labels:labels,datasets:[{label:lDep,data:depositsData,backgroundColor:window.cssToken('--color-navy-mid'),borderWidth:0,stack:'s'},{label:lGrow,data:growthData,backgroundColor:window.cssToken('--color-gold'),borderWidth:0,stack:'s'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){return c.dataset.label+': '+window.fmtMoney(c.raw);}}}},scales:{x:{stacked:true,grid:{display:false},ticks:{color:window.cssToken('--color-muted'),font:{size:11},maxRotation:0}},y:{stacked:true,grid:{color:'rgba(0,0,0,0.05)'},ticks:{color:window.cssToken('--color-muted'),font:{size:11},callback:function(v){var sym=window.getCurrencySym();var _cfg=window.CURRENCY_CONFIG&&window.CURRENCY_CONFIG[window.APP_CURRENCY];var _loc=_cfg?_cfg.loc:'en-US';return v>=1000000?sym+new Intl.NumberFormat(_loc,{maximumFractionDigits:1}).format(v/1000000)+'M':v>=1000?sym+new Intl.NumberFormat(_loc,{maximumFractionDigits:0}).format(v/1000)+'k':sym+v;}}}}}});}else{ciChart.data.labels=labels;ciChart.data.datasets[0].label=lDep;ciChart.data.datasets[0].data=depositsData;ciChart.data.datasets[1].label=lGrow;ciChart.data.datasets[1].data=growthData;ciChart.update('none');}}
...
if(monthlySavings<=0){verdict.textContent=txV('rf-verdict-higher');verdict.className='rf-verdict rf-verdict--negative';}else if(breakEvenMonths>remaining*12){verdict.textContent=txV('rf-verdict-long');verdict.className='rf-verdict rf-verdict--warn';}else{var beText=document.getElementById('rf-result-breakeven').textContent;verdict.textContent=txV('rf-verdict-summary').replace('{monthly}',fmtRF(monthlySavings)).replace('{total}',fmtRF(totalSavings)).replace('{breakeven}',beText)+' '+txV('rf-verdict-good');verdict.className='rf-verdict rf-verdict--positive';}}
...
document.getElementById('settings-drawer').addEventListener('keydown',function(e){if(!this.classList.contains('open')||e.key!=='Tab')return;var focusable=Array.from(this.querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled])')).filter(function(el){return el.tabIndex!==-1;});var first=focusable[0],last=focusable[focusable.length-1];if(e.shiftKey){if(document.activeElement===first){e.preventDefault();last.focus();}}else{if(document.activeElement===last){e.preventDefault();first.focus();}}});
