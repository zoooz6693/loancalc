...
(function () {
	...
	var currentTab = 'mortgage',
		chartInstance = null,
		showAllRows = false,
		fullSchedule = [];
	var sAmount = document.getElementById('s-amount'),
		sRate = document.getElementById('s-rate'),
		sTerm = document.getElementById('s-term');
	var iAmount = document.getElementById('i-amount'),
		iRate = document.getElementById('i-rate'),
		iTerm = document.getElementById('i-term');
	function fmt(n) {
		return window.fmtMoney(n);
	}
	function fmtN(n) {
		var cfg = window.CURRENCY_CONFIG[window.APP_CURRENCY];
		var loc = cfg ? cfg.loc : 'en-US';
	...
	function buildSchedule(principal, annualRate, years) {
		var r = annualRate / 100 / 12;
		var n = years * 12;
		var res = calcLoan(principal, annualRate, years);
		...
		for (var y = 1; y <= years; y++) {
			...
			for (var m = 0; m < months; m++) {
				...
				var prin = monthly - interest;
				paidI += interest;
				paidP += prin;
				balance -= prin;
				if (balance < 0.005) balance = 0;
			}
			rows.push({year: y, start: start, paidP: paidP, paidI: paidI, end: Math.max(0, balance)});
		}
		return rows;
	}
	function renderTable(rows, max) {
		...
		var html = '';
		var lim = max || rows.length;
		for (var i = 0; i < Math.min(lim, rows.length); i++) {
			var r = rows[i];
			var endCls = r.end < 1 ? ' class="green"' : '';
			html += '<tr><td>' + r.year + '</td><td>' + fmt(r.start) + '</td><td class="green">' + fmt(r.paidP) + '</td><td class="red">' + fmt(r.paidI) + '</td><td' + endCls + '>' + (r.end < 1 ? fmt(0) : fmt(r.end)) + '</td></tr>';
		}
		tbody.innerHTML = html;
		var btn = document.getElementById('toggle-rows');
		if (rows.length <= 5) {
			btn.style.display = 'none';
		} else {
			btn.style.display = '';
			var lblAll = btn.dataset.showAll || 'Show all years';
			var lblLess = btn.dataset.showLess || 'Show less';
			btn.textContent = showAllRows ? lblLess : lblAll;
		}
	}
	function renderChart(principal, interest) {
		...
		var pPct = Math.round((principal / (principal + interest)) * 100);
		document.getElementById('chart-center-pct').textContent = pPct + '%';
		var ci = window._i18n_current;
		var lP = (ci && ci['lbl-principal-pct']) || 'Principal';
		var lI = (ci && ci['lbl-interest-pct']) || 'Interest';
		if (chartInstance) {
			chartInstance.data.labels = [lP, lI];
		...
		chartInstance = new Chart(canvas, {
			type: 'doughnut',
			data: {labels: [lP, lI], datasets: [{data: [principal, interest], backgroundColor: [window.cssToken('--color-navy'), window.cssToken('--color-gold')], borderWidth: 0, hoverOffset: 6}]},
			options: {
				responsive: true,
				...
				plugins: {
					...
					tooltip: {
						callbacks: {
							label: function (ctx) {
								return ctx.label + ': ' + fmt(ctx.raw) + ' (' + Math.round((ctx.raw / (principal + interest)) * 100) + '%)';
							}
						}
					}
				},
	...
	window.updateSliderFillGlobal = updateSliderFill;

	/* ── Global range slider fill delegation ── */
	document.addEventListener('input', function (e) {
		if (e.target.type === 'range' && window.updateSliderFillGlobal) window.updateSliderFillGlobal(e.target);
	});
	...
	function update() {
		var principal = parseFloat(sAmount.value) || 0;
		var rate = parseFloat(sRate.value) || 0;
		var years = parseInt(sTerm.value) || 0;
		iAmount.value = principal;
		iRate.value = fmtDec(rate, 1);
		iTerm.value = years;
		updateSliderFill(sAmount);
		updateSliderFill(sRate);
		updateSliderFill(sTerm);
		if (!principal || !rate || !years) {
			return;
		}
		var res = calcLoan(principal, rate, years);
		document.getElementById('result-monthly').textContent = fmt(res.monthly);
		if (window.fitText) window.fitText(document.getElementById('result-monthly'));
		document.getElementById('result-principal').textContent = fmt(principal);
		document.getElementById('result-interest').textContent = fmt(res.interest);
		document.getElementById('result-total').textContent = fmt(res.total);
		var payoffYear = new Date().getFullYear() + years;
		document.getElementById('result-year').textContent = payoffYear;
		var pPct = (principal / res.total) * 100;
		var iPct = 100 - pPct;
		document.getElementById('bar-p').style.transform = 'scaleX(' + pPct / 100 + ')';
		document.getElementById('bar-i').style.transform = 'scaleX(' + iPct / 100 + ')';
		document.getElementById('pct-p').textContent = Math.round(pPct) + '%';
		document.getElementById('pct-i').textContent = Math.round(iPct) + '%';
		var _cP = document.getElementById('c-principal');
		if (_cP) {
			_cP.textContent = fmt(principal);
			document.getElementById('c-interest').textContent = fmt(res.interest);
			document.getElementById('c-total').textContent = fmt(res.total);
			document.getElementById('c-year').textContent = payoffYear;
			document.getElementById('c-principal-pct').textContent = '(' + Math.round(pPct) + '%)';
			document.getElementById('c-interest-pct').textContent = '(' + Math.round(iPct) + '%)';
		}
		renderChart(principal, res.interest);
		fullSchedule = buildSchedule(principal, rate, years);
		renderTable(fullSchedule, showAllRows ? null : 5);
	}
	function switchTab(tab) {
		...
		var _ltp = document.getElementById('loan-type-content');
		if (_ltp) _ltp.setAttribute('aria-labelledby', 'loan-tab-' + tab);
		sAmount.min = cfg.amountMin;
		sAmount.max = cfg.amountMax;
		sAmount.step = cfg.amountStep;
		sAmount.value = cfg.amount;
		sRate.min = cfg.rateMin;
		sRate.max = cfg.rateMax;
		sRate.value = cfg.rate;
		sTerm.min = cfg.termMin;
		sTerm.max = cfg.termMax;
		sTerm.value = cfg.term;
		var _sym = window.getCurrencySym();
		document.getElementById('amount-min-lbl').textContent = _sym + fmtN(cfg.amountMin);
		document.getElementById('amount-max-lbl').textContent = _sym + fmtN(cfg.amountMax);
		document.getElementById('rate-min-lbl').textContent = cfg.rateMin + '%';
		document.getElementById('rate-max-lbl').textContent = cfg.rateMax + '%';
		var txYr = (window._i18n_current && window._i18n_current['unit-yr']) || 'yr';
		var txYrs = (window._i18n_current && window._i18n_current['unit-yrs']) || 'yrs';
		document.getElementById('term-min-lbl').textContent = cfg.termMin + ' ' + txYr;
		document.getElementById('term-max-lbl').textContent = cfg.termMax + ' ' + txYrs;
		var txL = function (k) {
			return (window._i18n_current && window._i18n_current[k]) || cfg.label;
		};
		document.getElementById('result-loan-label').textContent = txL('loan-label-' + tab);
		document.getElementById('helper-title').textContent = txL('helper-title-' + tab) || cfg.helperTitle;
		document.getElementById('helper-text').textContent = txL('helper-text-' + tab) || cfg.helperText;
		showAllRows = false;
		update();
	}
	function applyCurrency(code) {
		var cfg = window.CURRENCY_CONFIG[code];
		if (!cfg) {
			code = 'USD';
			cfg = window.CURRENCY_CONFIG['USD'];
		}
		window.APP_CURRENCY = code;
		document.querySelectorAll('.ig-sym').forEach(function (el) {
			el.textContent = cfg.sym;
		});
		var ratios = {mortgage: {amtMul: 1, rateMul: 1, termDef: 30}, car: {amtMul: 0.08, rateMul: 1.15, termDef: 5}, personal: {amtMul: 0.033, rateMul: 1.7, termDef: 3}, student: {amtMul: 0.12, rateMul: 0.85, termDef: 10}};
		['mortgage', 'car', 'personal', 'student'].forEach(function (tab) {
			var r = ratios[tab];
			var amt = Math.round(cfg.lDef * r.amtMul);
			var step = Math.max(1, Math.round(cfg.lStep * r.amtMul));
			CONFIGS[tab].amount = amt;
			CONFIGS[tab].amountMin = Math.round(cfg.lMin * r.amtMul) || cfg.lMin;
			CONFIGS[tab].amountMax = Math.round(cfg.lMax * r.amtMul);
			CONFIGS[tab].amountStep = step;
			CONFIGS[tab].rate = Math.round(cfg.rDef * r.rateMul * 10) / 10;
			CONFIGS[tab].rateMax = Math.round(cfg.rMax * r.rateMul);
		});
		CONFIGS.mortgage.helperTitle = code + ' mortgage market';
		CONFIGS.mortgage.helperText = cfg.note;
		switchTab(currentTab);
		var ciP = document.getElementById('ci-s-principal');
		var ciR = document.getElementById('ci-s-rate');
		var ciM = document.getElementById('ci-s-monthly');
		if (ciP) {
			var ciMonthly = Math.round(cfg.ciDef * 0.02);
			ciP.max = cfg.lMax;
			ciP.step = cfg.lStep;
			ciP.value = cfg.ciDef;
			document.getElementById('ci-i-principal').value = cfg.ciDef;
			ciM.max = Math.round(cfg.lStep * 2);
			ciM.value = ciMonthly;
			document.getElementById('ci-i-monthly').value = ciMonthly;
			var _ciPMinEl = document.getElementById('ci-p-min-lbl');
			var _ciPMaxEl = document.getElementById('ci-p-max-lbl');
			if (_ciPMinEl) _ciPMinEl.textContent = cfg.sym + fmtN(ciP.min || 100);
			if (_ciPMaxEl) _ciPMaxEl.textContent = cfg.sym + fmtN(cfg.lMax);
			var _ciMMinEl = document.getElementById('ci-m-min-lbl');
			var _ciMMaxEl = document.getElementById('ci-m-max-lbl');
			if (_ciMMinEl) _ciMMinEl.textContent = cfg.sym + '0';
			if (_ciMMaxEl) _ciMMaxEl.textContent = cfg.sym + fmtN(Math.round(cfg.lStep * 2)) + '/mo';
		}
		if (ciR) {
			ciR.value = cfg.ciRate;
			document.getElementById('ci-i-rate').value = cfg.ciRate;
		}
		if (window.calcCI) window.calcCI();
		var rfB = document.getElementById('rf-s-balance');
		var rfO = document.getElementById('rf-s-oldrate');
		var rfN = document.getElementById('rf-s-newrate');
		if (rfB) {
			var rfBal = Math.round(cfg.lDef * 0.83);
			rfB.min = cfg.lMin;
			rfB.max = cfg.lMax;
			rfB.step = cfg.lStep;
			rfB.value = rfBal;
			document.getElementById('rf-i-balance').value = rfBal;
			var _rfBalMinEl = document.getElementById('rf-bal-min-lbl');
			var _rfBalMaxEl = document.getElementById('rf-bal-max-lbl');
			if (_rfBalMinEl) _rfBalMinEl.textContent = cfg.sym + fmtN(cfg.lMin);
			if (_rfBalMaxEl) _rfBalMaxEl.textContent = cfg.sym + fmtN(cfg.lMax);
		}
		if (rfO) {
			var oldR = Math.round(cfg.rDef * 115) / 100;
			rfO.value = oldR;
			document.getElementById('rf-i-oldrate').value = oldR;
		}
		if (rfN) {
			rfN.value = cfg.rDef;
			document.getElementById('rf-i-newrate').value = cfg.rDef;
		}
		if (window.calcRF) window.calcRF();
		if (window._setCurrencyPref) window._setCurrencyPref(code);
		if (window.updateCommodityLocal) window.updateCommodityLocal();
	...
	var _loanRaf = null;
	function updateRaf() {
		if (_loanRaf) cancelAnimationFrame(_loanRaf);
		_loanRaf = requestAnimationFrame(function () {
			_loanRaf = null;
			update();
		});
	}
	[sAmount, sRate, sTerm].forEach(function (el) {
		el.addEventListener('input', updateRaf);
	});
	iAmount.addEventListener('input', function () {
		var v = parseFloat(this.value);
		if (!isNaN(v) && v >= parseFloat(sAmount.min) && v <= parseFloat(sAmount.max)) {
			sAmount.value = v;
			update();
		}
	});
	iRate.addEventListener('input', function () {
		var v = parseFloat(this.value);
		if (!isNaN(v) && v >= parseFloat(sRate.min) && v <= parseFloat(sRate.max)) {
			sRate.value = v;
			update();
		}
	});
	iTerm.addEventListener('input', function () {
		var v = parseInt(this.value);
		if (!isNaN(v) && v >= parseInt(sTerm.min) && v <= parseInt(sTerm.max)) {
			sTerm.value = v;
			update();
		}
	});
	iAmount.addEventListener('blur', function () {
		var v = parseFloat(this.value);
		if (isNaN(v)) {
			this.value = sAmount.value;
		} else {
			var c = Math.min(parseFloat(sAmount.max), Math.max(parseFloat(sAmount.min), v));
			if (c !== v) {
				this.value = c;
				sAmount.value = c;
				update();
	...
	iRate.addEventListener('blur', function () {
		var v = parseFloat(this.value);
		if (isNaN(v)) {
			this.value = sRate.value;
		} else {
			var c = Math.min(parseFloat(sRate.max), Math.max(parseFloat(sRate.min), v));
			if (c !== v) {
	...
	iTerm.addEventListener('blur', function () {
		var v = parseInt(this.value);
		if (isNaN(v)) {
			this.value = sTerm.value;
		} else {
			var c = Math.min(parseInt(sTerm.max), Math.max(parseInt(sTerm.min), v));
			if (c !== v) {
				this.value = c;
				sTerm.value = c;
				update();
	...
	document.getElementById('toggle-rows').addEventListener('click', function () {
		...
		renderTable(fullSchedule, showAllRows ? null : 5);
		this.setAttribute('aria-expanded', showAllRows);
	});
...
(function () {
	...
	function calcCI() {
		var P = parseFloat(document.getElementById('ci-s-principal').value) || 0;
		var M = parseFloat(document.getElementById('ci-s-monthly').value) || 0;
		var r = (parseFloat(document.getElementById('ci-s-rate').value) || 0) / 100 / 12;
		var years = parseInt(document.getElementById('ci-s-years').value) || 0;
		var n = years * 12;
		var fv = P * Math.pow(1 + r, n);
		if (r > 0) {
			fv += (M * (Math.pow(1 + r, n) - 1)) / r;
		} else {
			fv += M * n;
		}
		var deposited = P + M * n;
		var earned = fv - deposited;
		var mult = deposited > 0 ? fv / deposited : 0;
		var targetYear = new Date().getFullYear() + years;
		document.getElementById('ci-result-fv').textContent = fmtCI(fv);
		if (window.fitText) window.fitText(document.getElementById('ci-result-fv'));
		document.getElementById('ci-result-deposited').textContent = fmtCI(deposited);
		document.getElementById('ci-result-earned').textContent = fmtCI(earned);
		var _mCfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[window.APP_CURRENCY];
		var _mLoc = _mCfg ? _mCfg.loc : 'en-US';
		document.getElementById('ci-result-mult').textContent = new Intl.NumberFormat(_mLoc, {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(mult) + '×';
		document.getElementById('ci-result-year').textContent = targetYear;
		document.getElementById('ci-result-years-lbl').textContent = years;
		var dPct = fv > 0 ? Math.round((deposited / fv) * 100) : 0;
		var gPct = 100 - dPct;
		document.getElementById('ci-bar-d').style.transform = 'scaleX(' + dPct / 100 + ')';
		document.getElementById('ci-bar-g').style.transform = 'scaleX(' + gPct / 100 + ')';
		document.getElementById('ci-pct-d').textContent = dPct + '%';
		document.getElementById('ci-pct-g').textContent = gPct + '%';
		updateCIChart(P, M, r, years);
	}
	function updateCIChart(P, M, r, years) {
		if (!window.Chart) return;
		var txCI = function (k) {
			return (window._i18n_current && window._i18n_current[k]) || k;
		};
		var lDep = txCI('ci-deposited');
		var lGrow = txCI('ci-earned-short');
		var lYr = txCI('unit-yr');
		var labels = [],
			depositsData = [],
			growthData = [];
		for (var y = 1; y <= years; y++) {
			...
			var dep = P + M * n;
			labels.push(y === 1 ? '1 ' + lYr : y % 5 === 0 ? y + ' ' + lYr : '');
			depositsData.push(Math.round(dep));
			growthData.push(Math.round(Math.max(0, fv - dep)));
		}
		if (!ciChart) {
			ciChart = new Chart(document.getElementById('ciChart'), {
				...
				data: {
					labels: labels,
					datasets: [
						{label: lDep, data: depositsData, backgroundColor: window.cssToken('--color-navy-mid'), borderWidth: 0, stack: 's'},
						{label: lGrow, data: growthData, backgroundColor: window.cssToken('--color-gold'), borderWidth: 0, stack: 's'}
					]
				},
				options: {
					responsive: true,
					...
					plugins: {
						...
						tooltip: {
							callbacks: {
								label: function (c) {
									return c.dataset.label + ': ' + window.fmtMoney(c.raw);
								}
							}
						}
					},
					scales: {
						x: {stacked: true, grid: {display: false}, ticks: {color: window.cssToken('--color-muted'), font: {size: 11}, maxRotation: 0}},
						y: {
							stacked: true,
							...
							ticks: {
								...
								font: {size: 11},
								callback: function (v) {
									var sym = window.getCurrencySym();
									var _cfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[window.APP_CURRENCY];
									var _loc = _cfg ? _cfg.loc : 'en-US';
									return v >= 1000000 ? sym + new Intl.NumberFormat(_loc, {maximumFractionDigits: 1}).format(v / 1000000) + 'M' : v >= 1000 ? sym + new Intl.NumberFormat(_loc, {maximumFractionDigits: 0}).format(v / 1000) + 'k' : sym + v;
								}
							}
						}
					}
			...
			ciChart.data.datasets[0].label = lDep;
			ciChart.data.datasets[0].data = depositsData;
			ciChart.data.datasets[1].label = lGrow;
			ciChart.data.datasets[1].data = growthData;
			ciChart.update('none');
		}
	}
	...
	['ci-s-principal', 'ci-s-monthly', 'ci-s-rate', 'ci-s-years'].forEach(function (id) {
		var slider = document.getElementById(id);
		var inputId = id.replace('ci-s-', 'ci-i-');
		slider.addEventListener('input', function () {
			document.getElementById(inputId).value = this.value;
			calcCI();
		});
	});
	['ci-i-principal', 'ci-i-monthly', 'ci-i-rate'].forEach(function (id) {
		document.getElementById(id).addEventListener('input', function () {
			var sliderId = id.replace('ci-i-', 'ci-s-');
	...
	document.getElementById('ci-i-years').addEventListener('input', function () {
		var sliderId = 'ci-s-years';
		var slider = document.getElementById(sliderId);
		var v = parseInt(this.value);
		if (!isNaN(v) && v >= parseInt(slider.min) && v <= parseInt(slider.max)) {
			slider.value = v;
			calcCI();
		}
	});
	...
	if (document.getElementById('ci-result-fv')) calcCI();
})();

(function () {
	'use strict';
	function fmtRF(n) {
		return window.fmtMoney(Math.abs(n));
	}
	function calcMonthly(bal, rate, years) {
		var r = rate / 100 / 12;
		var n = years * 12;
		if (!n) return 0;
		if (r === 0) return bal / n;
	...
	function calcRF() {
		var bal = parseFloat(document.getElementById('rf-s-balance').value) || 0;
		var oldRate = parseFloat(document.getElementById('rf-s-oldrate').value) || 0;
		var remaining = parseInt(document.getElementById('rf-s-remaining').value) || 0;
		var newRate = parseFloat(document.getElementById('rf-s-newrate').value) || 0;
		var costs = parseFloat(document.getElementById('rf-s-costs').value) || 0;
		var oldPayment = calcMonthly(bal, oldRate, remaining);
		var newPayment = calcMonthly(bal, newRate, remaining);
		var monthlySavings = oldPayment - newPayment;
		var totalSavings = monthlySavings * remaining * 12 - costs;
		var breakEvenMonths = monthlySavings > 0 ? Math.ceil(costs / monthlySavings) : Infinity;
		document.getElementById('rf-result-old').textContent = fmtRF(oldPayment);
		document.getElementById('rf-result-new').textContent = fmtRF(newPayment);
		document.getElementById('rf-result-monthly').textContent = (monthlySavings >= 0 ? '' : '-') + fmtRF(monthlySavings);
		if (window.fitText) window.fitText(document.getElementById('rf-result-monthly'));
		document.getElementById('rf-result-total').textContent = (totalSavings >= 0 ? '' : '-') + fmtRF(totalSavings);
		var txV = function (k) {
			return (window._i18n_rf && window._i18n_rf[k]) || k;
		};
		if (breakEvenMonths === Infinity || breakEvenMonths < 0) {
			document.getElementById('rf-result-breakeven').textContent = txV('rf-never');
		} else if (breakEvenMonths > remaining * 12) {
			document.getElementById('rf-result-breakeven').textContent = txV('rf-over-term').replace('{n}', remaining);
		} else {
			var beYears = Math.floor(breakEvenMonths / 12);
			var beMos = breakEvenMonths % 12;
			document.getElementById('rf-result-breakeven').textContent = beYears > 0 ? txV('rf-years-mo').replace('{y}', beYears).replace('{m}', beMos) : txV('rf-months').replace('{n}', beMos);
		}
		var verdict = document.getElementById('rf-verdict');
		if (monthlySavings <= 0) {
			verdict.textContent = txV('rf-verdict-higher');
			verdict.className = 'rf-verdict rf-verdict--negative';
		} else if (breakEvenMonths > remaining * 12) {
			verdict.textContent = txV('rf-verdict-long');
			verdict.className = 'rf-verdict rf-verdict--warn';
		} else {
			var beText = document.getElementById('rf-result-breakeven').textContent;
			verdict.textContent = txV('rf-verdict-summary').replace('{monthly}', fmtRF(monthlySavings)).replace('{total}', fmtRF(totalSavings)).replace('{breakeven}', beText) + ' ' + txV('rf-verdict-good');
			verdict.className = 'rf-verdict rf-verdict--positive';
		}
	}
	window.calcRF = calcRF;
	['balance', 'oldrate', 'newrate', 'costs'].forEach(function (k) {
		var s = document.getElementById('rf-s-' + k);
		var i = document.getElementById('rf-i-' + k);
		s.addEventListener('input', function () {
			i.value = this.value;
			calcRF();
		});
		i.addEventListener('input', function () {
			if (parseFloat(this.value) >= parseFloat(s.min) && parseFloat(this.value) <= parseFloat(s.max)) {
				s.value = this.value;
				calcRF();
			}
		});
		i.addEventListener('blur', function () {
			...
			var c = Math.min(parseFloat(s.max), Math.max(parseFloat(s.min), v));
			if (c !== v) {
				this.value = c;
				s.value = c;
				calcRF();
	...
	(function () {
		var s = document.getElementById('rf-s-remaining');
		var i = document.getElementById('rf-i-remaining');
		s.addEventListener('input', function () {
			i.value = this.value;
			calcRF();
		});
		i.addEventListener('input', function () {
			var v = parseInt(this.value);
			if (!isNaN(v) && v >= parseInt(s.min) && v <= parseInt(s.max)) {
				s.value = v;
				calcRF();
			}
		});
		i.addEventListener('blur', function () {
			...
			var c = Math.min(parseInt(s.max), Math.max(parseInt(s.min), v));
			if (c !== v) {
				this.value = c;
				s.value = c;
				calcRF();
...
(function () {
	...
	window.ALL_CURRENCIES = [
		{c: 'AED', n: 'UAE Dirham'},
		{c: 'AFN', n: 'Afghan Afghani'},
		{c: 'ALL', n: 'Albanian Lek'},
		{c: 'AMD', n: 'Armenian Dram'},
		{c: 'ANG', n: 'Netherlands Antillean Guilder'},
		{c: 'AOA', n: 'Angolan Kwanza'},
		{c: 'ARS', n: 'Argentine Peso'},
		{c: 'AUD', n: 'Australian Dollar'},
		{c: 'AWG', n: 'Aruban Florin'},
		{c: 'AZN', n: 'Azerbaijani Manat'},
		{c: 'BAM', n: 'Bosnian Mark'},
		{c: 'BBD', n: 'Barbadian Dollar'},
		{c: 'BDT', n: 'Bangladeshi Taka'},
		{c: 'BGN', n: 'Bulgarian Lev'},
		{c: 'BHD', n: 'Bahraini Dinar'},
		{c: 'BIF', n: 'Burundian Franc'},
		{c: 'BMD', n: 'Bermudan Dollar'},
		{c: 'BND', n: 'Brunei Dollar'},
		{c: 'BOB', n: 'Bolivian Boliviano'},
		{c: 'BRL', n: 'Brazilian Real'},
		{c: 'BSD', n: 'Bahamian Dollar'},
		{c: 'BTN', n: 'Bhutanese Ngultrum'},
		{c: 'BWP', n: 'Botswanan Pula'},
		{c: 'BYN', n: 'Belarusian Ruble'},
		{c: 'BZD', n: 'Belize Dollar'},
		{c: 'CAD', n: 'Canadian Dollar'},
		{c: 'CDF', n: 'Congolese Franc'},
		{c: 'CHF', n: 'Swiss Franc'},
		{c: 'CLP', n: 'Chilean Peso'},
		{c: 'CNY', n: 'Chinese Yuan'},
		{c: 'COP', n: 'Colombian Peso'},
		{c: 'CRC', n: 'Costa Rican Colón'},
		{c: 'CUP', n: 'Cuban Peso'},
		{c: 'CVE', n: 'Cape Verdean Escudo'},
		{c: 'CZK', n: 'Czech Koruna'},
		{c: 'DJF', n: 'Djiboutian Franc'},
		{c: 'DKK', n: 'Danish Krone'},
		{c: 'DOP', n: 'Dominican Peso'},
		{c: 'DZD', n: 'Algerian Dinar'},
		{c: 'EGP', n: 'Egyptian Pound'},
		{c: 'ERN', n: 'Eritrean Nakfa'},
		{c: 'ETB', n: 'Ethiopian Birr'},
		{c: 'EUR', n: 'Euro'},
		{c: 'FJD', n: 'Fijian Dollar'},
		{c: 'FKP', n: 'Falkland Islands Pound'},
		{c: 'GBP', n: 'British Pound'},
		{c: 'GEL', n: 'Georgian Lari'},
		{c: 'GHS', n: 'Ghanaian Cedi'},
		{c: 'GIP', n: 'Gibraltar Pound'},
		{c: 'GMD', n: 'Gambian Dalasi'},
		{c: 'GNF', n: 'Guinean Franc'},
		{c: 'GTQ', n: 'Guatemalan Quetzal'},
		{c: 'GYD', n: 'Guyanese Dollar'},
		{c: 'HKD', n: 'Hong Kong Dollar'},
		{c: 'HNL', n: 'Honduran Lempira'},
		{c: 'HTG', n: 'Haitian Gourde'},
		{c: 'HUF', n: 'Hungarian Forint'},
		{c: 'IDR', n: 'Indonesian Rupiah'},
		{c: 'ILS', n: 'Israeli Shekel'},
		{c: 'INR', n: 'Indian Rupee'},
		{c: 'IQD', n: 'Iraqi Dinar'},
		{c: 'IRR', n: 'Iranian Rial'},
		{c: 'ISK', n: 'Icelandic Króna'},
		{c: 'JMD', n: 'Jamaican Dollar'},
		{c: 'JOD', n: 'Jordanian Dinar'},
		{c: 'JPY', n: 'Japanese Yen'},
		{c: 'KES', n: 'Kenyan Shilling'},
		{c: 'KGS', n: 'Kyrgyzstani Som'},
		{c: 'KHR', n: 'Cambodian Riel'},
		{c: 'KMF', n: 'Comorian Franc'},
		{c: 'KRW', n: 'South Korean Won'},
		{c: 'KWD', n: 'Kuwaiti Dinar'},
		{c: 'KYD', n: 'Cayman Islands Dollar'},
		{c: 'KZT', n: 'Kazakhstani Tenge'},
		{c: 'LAK', n: 'Laotian Kip'},
		{c: 'LBP', n: 'Lebanese Pound'},
		{c: 'LKR', n: 'Sri Lankan Rupee'},
		{c: 'LRD', n: 'Liberian Dollar'},
		{c: 'LSL', n: 'Lesotho Loti'},
		{c: 'LYD', n: 'Libyan Dinar'},
		{c: 'MAD', n: 'Moroccan Dirham'},
		{c: 'MDL', n: 'Moldovan Leu'},
		{c: 'MGA', n: 'Malagasy Ariary'},
		{c: 'MKD', n: 'Macedonian Denar'},
		{c: 'MMK', n: 'Myanmar Kyat'},
		{c: 'MNT', n: 'Mongolian Tögrög'},
		{c: 'MOP', n: 'Macanese Pataca'},
		{c: 'MRU', n: 'Mauritanian Ouguiya'},
		{c: 'MUR', n: 'Mauritian Rupee'},
		{c: 'MVR', n: 'Maldivian Rufiyaa'},
		{c: 'MWK', n: 'Malawian Kwacha'},
		{c: 'MXN', n: 'Mexican Peso'},
		{c: 'MYR', n: 'Malaysian Ringgit'},
		{c: 'MZN', n: 'Mozambican Metical'},
		{c: 'NAD', n: 'Namibian Dollar'},
		{c: 'NGN', n: 'Nigerian Naira'},
		{c: 'NIO', n: 'Nicaraguan Córdoba'},
		{c: 'NOK', n: 'Norwegian Krone'},
		{c: 'NPR', n: 'Nepalese Rupee'},
		{c: 'NZD', n: 'New Zealand Dollar'},
		{c: 'OMR', n: 'Omani Rial'},
		{c: 'PAB', n: 'Panamanian Balboa'},
		{c: 'PEN', n: 'Peruvian Sol'},
		{c: 'PGK', n: 'Papua New Guinean Kina'},
		{c: 'PHP', n: 'Philippine Peso'},
		{c: 'PKR', n: 'Pakistani Rupee'},
		{c: 'PLN', n: 'Polish Złoty'},
		{c: 'PYG', n: 'Paraguayan Guaraní'},
		{c: 'QAR', n: 'Qatari Riyal'},
		{c: 'RON', n: 'Romanian Leu'},
		{c: 'RSD', n: 'Serbian Dinar'},
		{c: 'RUB', n: 'Russian Ruble'},
		{c: 'RWF', n: 'Rwandan Franc'},
		{c: 'SAR', n: 'Saudi Riyal'},
		{c: 'SBD', n: 'Solomon Islands Dollar'},
		{c: 'SCR', n: 'Seychellois Rupee'},
		{c: 'SDG', n: 'Sudanese Pound'},
		{c: 'SEK', n: 'Swedish Krona'},
		{c: 'SGD', n: 'Singapore Dollar'},
		{c: 'SHP', n: 'Saint Helena Pound'},
		{c: 'SOS', n: 'Somali Shilling'},
		{c: 'SRD', n: 'Surinamese Dollar'},
		{c: 'STN', n: 'São Tomé Dobra'},
		{c: 'SZL', n: 'Swazi Lilangeni'},
		{c: 'THB', n: 'Thai Baht'},
		{c: 'TJS', n: 'Tajikistani Somoni'},
		{c: 'TMT', n: 'Turkmenistan Manat'},
		{c: 'TND', n: 'Tunisian Dinar'},
		{c: 'TOP', n: 'Tongan Paʻanga'},
		{c: 'TRY', n: 'Turkish Lira'},
		{c: 'TTD', n: 'Trinidad Dollar'},
		{c: 'TWD', n: 'New Taiwan Dollar'},
		{c: 'TZS', n: 'Tanzanian Shilling'},
		{c: 'UAH', n: 'Ukrainian Hryvnia'},
		{c: 'UGX', n: 'Ugandan Shilling'},
		{c: 'USD', n: 'US Dollar'},
		{c: 'UYU', n: 'Uruguayan Peso'},
		{c: 'UZS', n: 'Uzbekistani Som'},
		{c: 'VES', n: 'Venezuelan Bolívar'},
		{c: 'VND', n: 'Vietnamese Dong'},
		{c: 'VUV', n: 'Vanuatu Vatu'},
		{c: 'WST', n: 'Samoan Tala'},
		{c: 'XAF', n: 'Central African Franc'},
		{c: 'XCD', n: 'East Caribbean Dollar'},
		{c: 'XOF', n: 'West African Franc'},
		{c: 'XPF', n: 'CFP Franc'},
		{c: 'YER', n: 'Yemeni Rial'},
		{c: 'ZAR', n: 'South African Rand'},
		{c: 'ZMW', n: 'Zambian Kwacha'},
	...
	var I18N = {
		en: {
			...
			'helper-title-mortgage': 'Typical 30-year US mortgage',
			'helper-title-car': 'Typical car loan',
			'helper-title-personal': 'Typical personal loan',
			'helper-title-student': 'Federal student loan (US)',
			...
			'ci-helper-title': 'S&P 500 historical average',
			'ci-helper-text': 'The US stock market has returned ~7% annually after inflation over the long term. Use 7% as a realistic baseline for diversified index funds.',
		...
		ar: {
			...
			'hero-sub': 'حاسبة القروض، الفائدة المركبة، إعادة التمويل، محول العملات المباشر، سعر الذهب والنفط: مجاناً، فورياً، دون تسجيل.',
			...
			'helper-title-mortgage': 'قرض عقاري مصري نموذجي',
			'helper-title-car': 'قرض سيارة نموذجي',
			'helper-title-personal': 'قرض شخصي نموذجي',
			'helper-title-student': 'قرض طلابي نموذجي',
			'helper-text-mortgage': 'معدل البنك المركزي المصري: 27.25٪. القروض العقارية للمستهلكين: 18-25٪.',
			'helper-text-car': 'متوسط معدل قرض السيارة الجديدة 6-8٪. السيارات المستعملة عادةً أعلى قليلاً.',
			'helper-text-personal': 'معدلات القروض الشخصية تتراوح بين 6٪ (تصنيف ممتاز) و36٪ (تصنيف متوسط).',
			'helper-text-student': 'معدلات القروض الطلابية الفيدرالية تحددها الحكومة سنوياً.',
		...
		fr: {
			...
			'hero-sub': "Calculateur de prêts, intérêts composés, refinancement, convertisseur de devises, prix de l'or et du pétrole: gratuit, instantané.",
		...
		es: {
			...
			'hero-sub': 'Calculadora de préstamos, interés compuesto, refinanciación, conversor de divisas, precio del oro y del petróleo: gratis, instantáneo.',
		...
		de: {
			...
			'hero-sub': 'Kreditrechner, Zinseszins, Refinanzierung, Währungsrechner, Gold- und Ölpreis: kostenlos, sofort, ohne Anmeldung.',
		...
		zh: {
			...
			'pref-title': '偏好设置',
			'pref-save': '保存设置',
			'hero-sub': '贷款计算器、复利、再融资、实时货币转换器、黄金和石油价格：免费、即时、无需注册。',
			...
			'trust-2': '无需注册',
			'trust-3': '全球通用',
			'hero-sub': '贷款计算器、复利、再融资节省、实时货币转换器、黄金和石油价格：免费、即时、无需注册。',
		...
		hi: {
			...
			'hero-sub': 'ऋण कैलकुलेटर, चक्रवृद्धि ब्याज, पुनर्वित्त, लाइव मुद्रा कनवर्टर, सोने और तेल की कीमत: सब मुफ़्त।',
			...
			'hero-sub': 'ऋण कैलकुलेटर, चक्रवृद्धि ब्याज, पुनर्वित्त बचत, लाइव मुद्रा कनवर्टर, सोने और तेल की कीमत: सब मुफ़्त।',
		...
		pt: {
			...
			'hero-sub': 'Calculadora de empréstimos, juros compostos, refinanciamento, conversor de moedas, preço do ouro e petróleo: gratuito, instantâneo.',
			...
			'hero-sub': 'Calculadora de empréstimos, juros compostos, refinanciamento, conversor de moedas ao vivo, preço do ouro e petróleo: gratuito, instantâneo.',
		...
		tr: {
			...
			'hero-sub': 'Kredi hesaplayıcı, bileşik faiz, refinansman, canlı döviz çevirici, altın ve petrol fiyatları: ücretsiz, anında.',
			...
			'hero-sub': 'Kredi hesaplayıcı, bileşik faiz, refinansman tasarrufları, canlı döviz çevirici, altın ve petrol fiyatları: ücretsiz, anında.',
		...
		id: {
			...
			'hero-sub': 'Kalkulator pinjaman, bunga majemuk, refinansi, konverter mata uang langsung, harga emas dan minyak: gratis, instan.',
			...
			'hero-sub': 'Kalkulator pinjaman, bunga majemuk, penghematan refinansi, konverter mata uang langsung, harga emas dan minyak: gratis, instan.',
	...
	(function () {
		...
		Object.keys(FA).forEach(function (l) {
			if (I18N[l]) {
				var o = FA[l];
				for (var k in o) I18N[l][k] = o[k];
			}
		});
	})();
	(function () {
		...
		Object.keys(CS).forEach(function (l) {
			if (I18N[l]) {
				var o = CS[l];
				for (var k in o) I18N[l][k] = o[k];
			}
	...
	(function () {
		...
		Object.keys(GT).forEach(function (l) {
			if (I18N[l]) {
				var o = GT[l];
				for (var k in o) I18N[l][k] = o[k];
			}
	...
	(function () {
		...
		Object.keys(CH).forEach(function (l) {
			if (I18N[l]) {
				var o = CH[l];
				for (var k in o) I18N[l][k] = o[k];
			}
	...
	(function () {
		var CL = {ar: {'clamp-min': 'الحد الأدنى:', 'clamp-max': 'الحد الأقصى:'}, fr: {'clamp-min': 'Minimum :', 'clamp-max': 'Maximum :'}, es: {'clamp-min': 'Mínimo:', 'clamp-max': 'Máximo:'}, de: {'clamp-min': 'Minimum:', 'clamp-max': 'Maximum:'}, zh: {'clamp-min': '最小值：', 'clamp-max': '最大值：'}, hi: {'clamp-min': 'न्यूनतम:', 'clamp-max': 'अधिकतम:'}, pt: {'clamp-min': 'Mínimo:', 'clamp-max': 'Máximo:'}, tr: {'clamp-min': 'Minimum:', 'clamp-max': 'Maksimum:'}, id: {'clamp-min': 'Minimum:', 'clamp-max': 'Maksimum:'}};
	...
	function applyLang(lang) {
		...
		document.querySelectorAll('[data-i18n]').forEach(function (el) {
			var v = tx(el.getAttribute('data-i18n'));
			if (v) el.textContent = v;
		});
		var h1 = document.getElementById('hero-h1');
		if (h1 && tx('hero-h1')) h1.innerHTML = tx('hero-h1');
		var sub = document.querySelector('.hero-sub');
		if (sub && tx('hero-sub')) sub.textContent = tx('hero-sub');
		var tabs = document.querySelectorAll('.tab-btn[data-tab]');
		var tabKeys = ['tab-mortgage', 'tab-car', 'tab-personal', 'tab-student'];
		tabs.forEach(function (btn, i) {
			if (tx(tabKeys[i])) btn.textContent = tx(tabKeys[i]);
		});
		document.querySelectorAll('.footer-links a[data-i18n]').forEach(function (a) {
			var v = tx(a.getAttribute('data-i18n'));
			if (v) a.textContent = v;
		});
		var hs = document.getElementById('hero-sub-el');
		if (hs && tx('hero-sub')) hs.textContent = tx('hero-sub');
		function setLabel(forAttr, key) {
			var el = document.querySelector('label[for="' + forAttr + '"]');
			if (el && tx(key)) el.textContent = tx(key);
		}
		setLabel('s-amount', 'lbl-amount');
		setLabel('s-rate', 'lbl-rate');
		setLabel('s-term', 'lbl-term');
		setLabel('ci-s-principal', 'ci-label-principal');
		setLabel('ci-s-monthly', 'ci-label-monthly');
		setLabel('ci-s-rate', 'ci-label-rate');
		setLabel('ci-s-years', 'ci-label-years');
		setLabel('rf-s-balance', 'rf-balance');
		setLabel('rf-s-oldrate', 'rf-oldrate');
		setLabel('rf-s-remaining', 'rf-remaining');
		setLabel('rf-s-newrate', 'rf-newrate');
		setLabel('rf-s-costs', 'rf-costs');
		function setEl(id, key) {
			var el = document.getElementById(id);
			if (el && tx(key)) el.textContent = tx(key);
		}
		setEl('res-monthly-lbl', 'res-monthly');
		setEl('res-principal-lbl', 'res-principal');
		setEl('res-interest-lbl', 'res-interest');
		setEl('res-total-lbl', 'res-total');
		setEl('res-year-lbl', 'res-year');
		var bpEl = document.getElementById('bp-label');
		if (bpEl && tx('lbl-principal-pct')) bpEl.textContent = tx('lbl-principal-pct') + ' ';
		var biEl = document.getElementById('bi-label');
		if (biEl && tx('lbl-interest-pct')) biEl.textContent = tx('lbl-interest-pct') + ' ';
		setEl('section-breakdown-h', 'section-breakdown');
		setEl('section-amort-h', 'section-amort');
		var thEls = document.querySelectorAll('#amort-table thead th');
		var thKeys = ['amort-year', 'amort-start', 'amort-ppaid', 'amort-ipaid', 'amort-end'];
		thEls.forEach(function (th, i) {
			if (tx(thKeys[i])) th.textContent = tx(thKeys[i]);
		});
		var toggleBtn = document.getElementById('toggle-rows');
		if (toggleBtn) {
			toggleBtn.dataset.showAll = tx('btn-show-all');
			toggleBtn.dataset.showLess = tx('btn-show-less');
		}
		setEl('how-h2', 'section-how');
		setEl('compound-h2', 'ci-h2');
		setLabel('ci-s-principal', 'ci-label-principal');
		setLabel('ci-s-monthly', 'ci-label-monthly');
		setLabel('ci-s-rate', 'ci-label-rate');
		setLabel('ci-s-years', 'ci-label-years');
		setEl('ci-result-lbl', 'ci-result-label');
		setEl('ci-sub-lbl', 'ci-sub');
		setEl('ci-deposited-lbl', 'ci-deposited');
		setEl('ci-earned-lbl', 'ci-earned');
		setEl('ci-mult-lbl', 'ci-mult');
		setEl('ci-year-lbl', 'ci-year');
		setEl('ci-chart-h', 'ci-chart-h');
		setEl('ci-helper-title', 'ci-helper-title');
		setEl('ci-helper-text', 'ci-helper-text');
		setEl('rf-h2', 'rf-h2');
		setEl('rf-desc', 'rf-desc');
		setEl('rf-current-lbl', 'rf-current');
		setEl('rf-new-lbl', 'rf-new');
		setLabel('rf-s-balance', 'rf-balance');
		setLabel('rf-s-oldrate', 'rf-oldrate');
		setLabel('rf-s-remaining', 'rf-remaining');
		setLabel('rf-s-newrate', 'rf-newrate');
		setLabel('rf-s-costs', 'rf-costs');
		setEl('rf-monthly-lbl', 'rf-monthly');
		setEl('rf-old-lbl', 'rf-old');
		setEl('rf-new-pay-lbl', 'rf-new-pay');
		setEl('rf-breakeven-lbl', 'rf-breakeven');
		setEl('rf-total-lbl', 'rf-total');
		setEl('cur-h2', 'cur-h2');
		setEl('tx-cur-sub', 'cur-section-sub');
		var amtLbl = document.querySelector('label[for="cur-from"]');
		if (amtLbl && tx('cur-amount-label')) amtLbl.textContent = tx('cur-amount-label');
		var toLbl = document.querySelector('label[for="cur-to"]');
		if (toLbl && tx('cur-to-label')) toLbl.textContent = tx('cur-to-label');
		setEl('cur-quick-lbl', 'cur-quick');
		setEl('gold-title-el', 'gold-title');
		setEl('gold-sub-el', 'gold-sub');
		setEl('oil-title-el', 'oil-title');
		setEl('oil-sub-el', 'oil-sub');
		setEl('gold-local-lbl', 'gold-local-lbl');
		setEl('oil-local-lbl', 'oil-local-lbl');
		var faqH = document.getElementById('faq-h2');
		if (faqH && tx('faq-heading')) faqH.textContent = tx('faq-heading');
		setEl('footer-desc-el', 'footer-desc');
		setEl('footer-disclaimer-el', 'footer-disclaimer');
		...
		document.querySelectorAll('.faq-q[data-i18n-question]').forEach(function (btn) {
			...
			var v = tx(k);
			if (v && v !== k) btn.childNodes[0].textContent = v;
		});
	...
	function populatePrefCurrencySelect(currentCur, filter) {
		var sel = document.getElementById('pref-currency');
		var list = window.ALL_CURRENCIES.filter(function (cur) {
			return !!window.CURRENCY_CONFIG[cur.c];
		});
		if (filter) {
			var q = filter.toLowerCase();
			list = list.filter(function (cur) {
				return cur.c.toLowerCase().indexOf(q) !== -1 || cur.n.toLowerCase().indexOf(q) !== -1;
			});
		}
		sel.innerHTML = '';
		list.forEach(function (cur) {
			...
			opt.value = cur.c;
			opt.textContent = cur.c + ': ' + cur.n;
			if (cur.c === currentCur) opt.selected = true;
			sel.appendChild(opt);
		});
		if (sel.value !== currentCur && !filter) sel.value = 'USD';
	}
	function openSettings() {
		var prefs = loadPrefs();
		document.getElementById('pref-lang').value = prefs.lang || 'en';
		var srch = document.getElementById('pref-currency-search');
		if (srch) srch.value = '';
		populatePrefCurrencySelect(prefs.currency || window._prefCurrency || 'USD');
	...
	document.getElementById('settings-save-btn').addEventListener('click', function () {
		var lang = document.getElementById('pref-lang').value;
		var currency = document.getElementById('pref-currency').value;
		savePrefs({lang: lang, currency: currency});
		window._prefCurrency = currency;
		applyLang(lang);
		if (window.applyCurrency) window.applyCurrency(currency);
		document.getElementById('pref-cur-tag').textContent = currency;
		document.getElementById('pref-lang-tag').textContent = lang.toUpperCase();
		closeSettings();
		document.getElementById('settings-open-btn').focus();
	...
	document.getElementById('settings-drawer').addEventListener('keydown', function (e) {
		if (!this.classList.contains('open') || e.key !== 'Tab') return;
		var focusable = Array.from(this.querySelectorAll('button:not([disabled]),input:not([disabled]),select:not([disabled])')).filter(function (el) {
	...
	applyLang(_initLang);
	if (window.applyCurrency) window.applyCurrency(_initCur);
...
(function () {
	...
	var rates = null,
		...
		rateCount = 0,
		fromCur = 'USD',
		toCur = detectLocaleCurrency();
	var CACHE_FX = 'loancalc_fx',
		goldUSD = null,
		oilUSD = null;
	...
	function fmtLocal(n, cur) {
		if (!n) return '—';
		var cfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[cur];
		var loc = cfg ? cfg.loc : 'en-US';
		return n.toLocaleString(loc, {minimumFractionDigits: 2, maximumFractionDigits: 2}) + ' ' + cur;
	}
	function txCur(k, v) {
		var t = window._i18n_current;
		var e = window._i18n_en;
		var s = (t && t[k]) || (e && e[k]) || k;
		return v ? s.replace('{date}', v) : s;
	}
	...
	function setDot(id, state) {
		var el = document.getElementById(id);
		if (!el) return;
		var c = {live: 'var(--color-gain)', stale: 'var(--color-gold)', loading: 'var(--color-border)', error: 'var(--color-loss)'};
		el.style.background = c[state] || c.loading;
		el.dataset.status = state || 'loading';
	}
	...
	function spinBtn(id) {
		...
		svg.style.animation = 'spin-refresh .65s cubic-bezier(0.4,0,0.2,1) 1';
	}

	function getAgeText(ts) {
		if (!ts) return null;
	...
	function updateAgeTexts() {
		var now = Date.now();
		if (fetchedAt.fx && rates) {
			var age = getAgeText(fetchedAt.fx);
			var stale = now - fetchedAt.fx > 6 * 3600000;
			setDot('cur-dot', stale ? 'stale' : 'live');
			var el = document.getElementById('cur-status-text');
			if (el) el.textContent = (stale ? 'Rates from ' + age : 'Updated ' + age) + ' \xB7 ' + rateCount + ' currencies';
		}
		if (fetchedAt.gold && goldUSD) {
			var age = getAgeText(fetchedAt.gold);
			var stale = now - fetchedAt.gold > 3600000;
			setDot('gold-dot', stale ? 'stale' : 'live');
			var el = document.getElementById('gold-status');
			if (el) el.textContent = stale ? 'From ' + age : 'Updated ' + age;
		}
		if (fetchedAt.oil && oilUSD) {
			var age = getAgeText(fetchedAt.oil);
			var stale = now - fetchedAt.oil > 3600000;
			setDot('oil-dot', stale ? 'stale' : 'live');
			var el = document.getElementById('oil-status');
			if (el) el.textContent = stale ? 'From ' + age : 'Updated ' + age;
		}
	}

	function populateSelects() {
		['cur-from', 'cur-to'].forEach(function (id) {
			var sel = document.getElementById(id);
			if (!sel) return;
			var currentVal = sel.value;
			sel.innerHTML = '';
			window.ALL_CURRENCIES.forEach(function (cur) {
				var opt = document.createElement('option');
				opt.value = cur.c;
				opt.textContent = cur.c + ': ' + cur.n;
				sel.appendChild(opt);
			});
			sel.value = currentVal || (id === 'cur-from' ? fromCur : toCur);
		});
	}

	function convert() {
		if (!rates) return;
		var amount = Math.max(0, parseFloat(document.getElementById('cur-amount').value) || 0);
		fromCur = document.getElementById('cur-from').value;
		toCur = document.getElementById('cur-to').value;
		var fromRate = rates[fromCur] || null;
		var toRate = rates[toCur] || null;
		if (!fromRate || !toRate) {
			document.getElementById('cur-result').textContent = txCur('cur-rate-unavailable');
			document.getElementById('cur-rate-display').textContent = txCur('cur-not-in-feed');
			return;
		}
		var rate = toRate / fromRate;
		var result = amount * rate;
		var toCfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[toCur];
		var toLoc = toCfg ? toCfg.loc : 'en-US';
		document.getElementById('cur-result').textContent = result.toLocaleString(toLoc, {maximumFractionDigits: 4}) + ' ' + toCur;
		var _rateLoc = (window.CURRENCY_CONFIG[window.APP_CURRENCY] || {loc: 'en-US'}).loc;
		var fmtRate = function (n) {
			return new Intl.NumberFormat(_rateLoc, {minimumFractionDigits: 6, maximumFractionDigits: 6}).format(n);
		};
		document.getElementById('cur-rate-display').textContent = '1 ' + fromCur + ' = ' + fmtRate(rate) + ' ' + toCur;
		document.getElementById('cur-rate-inv').textContent = '1 ' + toCur + ' = ' + fmtRate(1 / rate) + ' ' + fromCur;
		document.getElementById('cur-rate-date').textContent = rateDate || txCur('cur-today');
		buildQuickGrid(fromCur, toCur, rate);
		updateCommodityLocal();
	}

	function buildQuickGrid(from, to, rate) {
		var usdEq = (rates['USD'] || 1) / (rates[from] || 1);
		var amounts = usdEq >= 3 ? [1, 10, 100] : usdEq >= 0.5 ? [1, 100, 1000] : usdEq >= 0.05 ? [10, 100, 1000] : usdEq >= 0.005 ? [100, 1000, 10000] : usdEq >= 0.0005 ? [1000, 10000, 100000] : [10000, 100000, 1000000];
		var fromCfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[from];
		var fromLoc = fromCfg ? fromCfg.loc : 'en-US';
		var toCfg = window.CURRENCY_CONFIG && window.CURRENCY_CONFIG[to];
		var toLoc = toCfg ? toCfg.loc : 'en-US';
		var grid = document.getElementById('cur-quick-grid');
		if (!grid) return;
		grid.innerHTML = '';
		amounts.forEach(function (a) {
			var div = document.createElement('div');
			div.style.cssText = 'background:var(--color-surface-raised);border:1px solid var(--color-border);border-radius:8px;padding:8px 12px';
			div.innerHTML = '<div style="font-size:11px;color:var(--color-muted)">' + a.toLocaleString(fromLoc) + ' ' + from + '</div><div style="font-size:13px;font-weight:500;color:var(--color-navy)">' + (a * rate).toLocaleString(toLoc, {maximumFractionDigits: 2}) + ' ' + to + '</div>';
			grid.appendChild(div);
		});
	}

	function updateCommodityLocal() {
		if (!rates) return;
		var localCur = toCur || detectLocaleCurrency();
		var usdToLocal = (rates[localCur] || 1) / (rates['USD'] || 1);
		var el;
		el = document.getElementById('gold-local-cur');
		if (el) el.textContent = localCur;
		el = document.getElementById('oil-local-cur');
		if (el) el.textContent = localCur;
		if (goldUSD) {
			var gG = goldUSD / 31.1035;
			el = document.getElementById('gold-usd');
			if (el) el.textContent = fmtUSD(goldUSD) + ' /oz';
			el = document.getElementById('gold-gram');
			if (el) el.textContent = fmtUSD(gG);
			el = document.getElementById('gold-10g');
			if (el) el.textContent = fmtUSD(gG * 10);
			el = document.getElementById('gold-kg');
			if (el) el.textContent = fmtUSD(gG * 1000);
			el = document.getElementById('gold-local');
			if (el) el.textContent = fmtLocal(goldUSD * usdToLocal, localCur) + ' /oz';
		}
		if (oilUSD) {
			el = document.getElementById('oil-usd');
			if (el) el.textContent = fmtUSD(oilUSD) + ' /bbl';
			el = document.getElementById('oil-5');
			if (el) el.textContent = fmtUSD(oilUSD * 5);
			el = document.getElementById('oil-10');
			if (el) el.textContent = fmtUSD(oilUSD * 10);
			el = document.getElementById('oil-100');
			if (el) el.textContent = fmtUSD(oilUSD * 100);
			el = document.getElementById('oil-local');
			if (el) el.textContent = fmtLocal(oilUSD * usdToLocal, localCur) + ' /bbl';
		}
	}
	window.updateCommodityLocal = updateCommodityLocal;
	window._setCurrencyPref = function (newCur) {
		toCur = newCur;
		var sel = document.getElementById('cur-to');
		if (sel) sel.value = newCur;
		convert();
		updateCommodityLocal();
	};
	...
	function finalizeFx(r, date, quality) {
		rates = r;
		rateDate = date;
		rateCount = Object.keys(r).length;
		fetchedAt.fx = Date.now();
		isFetching.fx = false;
		try {
			localStorage.setItem(CACHE_FX, JSON.stringify({rates: r, date: date, count: rateCount, ts: fetchedAt.fx}));
		} catch (e) {}
		var age = getAgeText(fetchedAt.fx);
		var sEl = document.getElementById('cur-status-text');
		if (quality === 'live') {
			setDot('cur-dot', 'live');
			if (sEl) sEl.textContent = 'Updated ' + age + ' \xB7 ' + rateCount + ' currencies';
		} else {
			setDot('cur-dot', 'stale');
			if (sEl) sEl.textContent = 'Partial rates \xB7 ' + rateCount + ' currencies \xB7 ECB data';
		}
		populateSelects();
		convert();
	}
	function fetchRates() {
		if (isFetching.fx) return;
		isFetching.fx = true;
		spinBtn('cur-refresh-btn');
		setDot('cur-dot', 'loading');
		var sEl = document.getElementById('cur-status-text');
		if (sEl) sEl.textContent = txCur('cur-status-fetching');
				...
				fetch('https://cdn.jsdelivr.net/npm/@fawazahmed0/currency-api@latest/v1/currencies/usd.json')
					...
					.catch(function () {
						fetch('https://api.frankfurter.app/latest?base=USD')
							...
							.then(function (data) {
								data.rates['USD'] = 1;
								finalizeFx(data.rates, data.date, 'partial');
							})
							.catch(function () {
								isFetching.fx = false;
								...
								setDot('cur-dot', 'error');
								if (sEl) sEl.textContent = txCur('cur-status-offline');
								populateSelects();
								convert();
							});
					});
	...
	function loadRates() {
		try {
			var cached = localStorage.getItem(CACHE_FX);
			if (cached) {
				var obj = JSON.parse(cached);
				var ageH = (Date.now() - obj.ts) / 3600000;
				if (ageH < 24) {
					rates = obj.rates;
					rateDate = obj.date;
					rateCount = obj.count || Object.keys(obj.rates).length;
					fetchedAt.fx = obj.ts;
					var stale = ageH >= 6;
					setDot('cur-dot', stale ? 'stale' : 'live');
					var age = getAgeText(obj.ts);
					var el = document.getElementById('cur-status-text');
					if (el) el.textContent = (stale ? 'Rates from ' + age : 'Updated ' + age) + ' \xB7 ' + rateCount + ' currencies';
					populateSelects();
					convert();
	...
	function loadCmdCache(type) {
		try {
			var c = localStorage.getItem('loancalc_cmd_' + type);
			if (c) return JSON.parse(c);
		} catch (e) {}
		try {
			var old = localStorage.getItem('loancalc_cmd');
			if (old) {
				var o = JSON.parse(old);
				return {price: type === 'gold' ? o.gold : o.oil, ts: o.ts, date: o.date};
			}
		} catch (e) {}
		return null;
	}
	function saveCmdCache(type, price, ts, date) {
		try {
			localStorage.setItem('loancalc_cmd_' + type, JSON.stringify({price: price, ts: ts, date: date}));
		} catch (e) {}
	}
	function loadCommodities() {
		...
		['gold', 'oil'].forEach(function (type) {
			...
			if (c && c.price && now - c.ts < 4 * 3600000) {
				if (type === 'gold') goldUSD = c.price;
				else oilUSD = c.price;
				fetchedAt[type] = c.ts;
				var stale = now - c.ts > 3600000;
				setDot(type + '-dot', stale ? 'stale' : 'live');
				var age = getAgeText(c.ts);
		...
		fetch('/api/commodity?ticker=' + encodeURIComponent(ticker))
			...
			.catch(function (err) {
				...
				if (cached && cached.price) {
					if (type === 'gold' && !goldUSD) goldUSD = cached.price;
					if (type === 'oil' && !oilUSD) oilUSD = cached.price;
					if (!fetchedAt[type]) fetchedAt[type] = cached.ts;

					setDot(type + '-dot', 'stale');
					var age = getAgeText(cached.ts);
					if (statusEl) statusEl.textContent = 'From ' + age;

					updateCommodityLocal();
				} else {
					setDot(type + '-dot', 'error');
					if (statusEl) statusEl.textContent = 'Price unavailable';
				}
			});
	}
	...
	document.getElementById('cur-amount').addEventListener('blur', function () {
		var v = parseFloat(this.value);
		var mx = parseFloat(this.max) || 999999999;
		if (isNaN(v) || v < 0) {
			this.value = 0;
			convert();
		} else if (v > mx) {
	...
	document.getElementById('cur-to').addEventListener('change', function () {
		convert();
		updateCommodityLocal();
	});
	document.getElementById('cur-swap').addEventListener('click', function () {
		var tmp = document.getElementById('cur-from').value;
		document.getElementById('cur-from').value = document.getElementById('cur-to').value;
		document.getElementById('cur-to').value = tmp;
		convert();
	});
	var _rb = document.getElementById('cur-retry-btn');
	if (_rb)
		_rb.addEventListener('click', function () {
			fetchRates();
		});
	var _crf = document.getElementById('cur-refresh-btn');
	if (_crf)
		_crf.addEventListener('click', function () {
			fetchRates();
		});
	var _grf = document.getElementById('gold-refresh-btn');
	if (_grf)
		_grf.addEventListener('click', function () {
			fetchCommodity('GC=F', 'gold');
		});
	var _orf = document.getElementById('oil-refresh-btn');
	if (_orf)
		_orf.addEventListener('click', function () {
			fetchCommodity('CL=F', 'oil');
		});
	...
	setInterval(function () {
		if (!isFetching.fx) fetchRates();
		if (!isFetching.gold) fetchCommodity('GC=F', 'gold');
		if (!isFetching.oil) fetchCommodity('CL=F', 'oil');
	}, 3600000);
...
/* Bind tool bar buttons — only intercept <button> elements; <a> links navigate naturally */
document.querySelectorAll('#tool-nav .tab-btn').forEach(function (btn) {
	btn.addEventListener('click', function () {
		if (this.tagName === 'BUTTON') switchPanel(this.dataset.panel);
	});
});
/* Footer links — only data-panel anchors that have no real href */
...
/* Path-based tab routing — runs before hash so URL path wins */
(function () {
	var path = window.location.pathname.replace(/\/$/, '');
	var pathMap = {
		'/mortgage-calculator': {panel: 'panel-loans', loanTab: 'mortgage'},
		'/loan-calculator': {panel: 'panel-loans', loanTab: 'personal'},
		'/savings-calculator': {panel: 'panel-savings', loanTab: null},
		'/refinance-calculator': {panel: 'panel-refinance', loanTab: null},
		'/currency-converter': {panel: 'panel-currency', loanTab: null}
	};
	var def = pathMap[path];
	if (def) {
		switchPanel(def.panel);
		if (def.loanTab && window.switchTab) window.switchTab(def.loanTab);
	}
})();
/* Hash routing */
...
setTimeout(function () {
	...
	if (d && d.dataset.status === 'loading') {
		...
		d.style.background = 'var(--color-gold)';
	...
	['gold', 'oil'].forEach(function (type) {
		...
		if (dot && dot.dataset.status === 'loading') {
			...
			dot.style.background = 'var(--color-gold)';
...
(function () {
	...
	function restoreState() {
		try {
			...
			var r = document.getElementById('s-rate');
			var t = document.getElementById('s-term');
			function inBounds(el, v) {
				return el && v && parseFloat(v) >= parseFloat(el.min) && parseFloat(v) <= parseFloat(el.max);
			}
			if (inBounds(a, s.amount)) a.value = s.amount;
			if (inBounds(r, s.rate)) r.value = s.rate;
			if (inBounds(t, s.term)) t.value = s.term;
			if (window.updateSliderFillGlobal) {
				[a, r, t].forEach(function (el) {
					if (el) window.updateSliderFillGlobal(el);
				});
			}
...
(function () {
	var pairs = [
		{
			...
			note: 'clamp-amount',
			fmt: function (v) {
				return window.getCurrencySym ? window.getCurrencySym() + v.toLocaleString() : v.toLocaleString();
			}
		},
		{
			num: 'i-rate',
	...
	pairs.forEach(function (p) {
		...
		inp.addEventListener('change', function () {
			...
			if (isNaN(v)) return;
			if (v < mn) {
				note.textContent = ((window._i18n_current && window._i18n_current['clamp-min']) || 'Minimum:') + ' ' + p.fmt(mn);
				note.classList.add('visible');
			} else if (v > mx) {
				note.textContent = ((window._i18n_current && window._i18n_current['clamp-max']) || 'Maximum:') + ' ' + p.fmt(mx);
				note.classList.add('visible');
			} else {
				note.textContent = '';
